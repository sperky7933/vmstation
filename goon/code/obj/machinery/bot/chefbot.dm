/mob/living/simple_animal/bot/chefbot
	name = "Dramatic Chef"
	...
	bot_type = CHEF_BOT
	model = "Chefbot"
	bot_core_type = /obj/machinery/bot_core/chefbot
	var/raging = 0
	var/list/calledout = list()
	//no_camera = 1

/mob/living/simple_animal/bot/chefbot/proc/do_step()
...
/mob/living/simple_animal/bot/chefbot/proc/point(var/target)
	visible_message("<b>[src]</b> points at [target].")
	if (istype(target, /atom))
		var/D = new /obj/effect/temp_visual/point(get_turf(target))
		spawn(25)
			qdel(D)

/mob/living/simple_animal/bot/chefbot/proc/drama()
	for (var/mob/M in hearers(7, src))
		M << sound('goon/sound/effects/dramatic.ogg', volume = 100) // F U C K temporary measure

/mob/living/simple_animal/bot/chefbot/speak(var/message)
	if (message)
		message = uppertext(message)
		..(message)

/mob/living/simple_animal/bot/chefbot/proc/why_is_it_bad()
...
/mob/living/simple_animal/bot/chefbot/proc/yell()
			...
			if (1)
				...
				if (somefucker)
					...
					if (somefucker)
						...
						else if (somefucker.getFireLoss() < 50)
...
/mob/living/simple_animal/bot/chefbot/emag_act(var/mob/user, var/obj/item/card/emag/E)
	if (!src.emagged)
		if(user)
			user << "<span class='danger'>You short out the restraining bolt on [src].</span>"
		src.emagged = 1
		return 1
	return 0
	...
	if (src.emagged == 1)
		if (user)
			user << "<span class='notice'>You repair [src]'s restraining bolt.</span>"
		src.emagged = 0
	return 1

/mob/living/simple_animal/bot/chefbot/attackby(obj/item/W as obj, mob/user as mob)
...
/mob/living/simple_animal/bot/chefbot/gib()
	return src.explode()

/mob/living/simple_animal/bot/chefbot/explode()
	src.on = 0
...
/obj/machinery/bot_core/chefbot
