	...
	switch (action)
		...
		if ("add")
			...
			if (get_stickyban_from_ckey(ckey))
				to_chat(usr, "<span class='adminnotice'>Error: Can not add a stickyban: User already has a current sticky ban</span>", handle_whitespace = TRUE)
				return
		...
		if ("remove")
			...
			if (!ban)
				to_chat(usr, "<span class='adminnotice'>Error: No sticky ban for [ckey] found!</span>", handle_whitespace = TRUE)
				return
			if (alert("Are you sure you want to remove the sticky ban on [ckey]?","Are you sure","Yes","No") == "No")
				return
			if (!get_stickyban_from_ckey(ckey))
				to_chat(usr, "<span class='adminnotice'>Error: The ban disappeared.</span>", handle_whitespace = TRUE)
				return
		...
		if ("remove_alt")
			...
			if (!key)
				to_chat(usr, "<span class='adminnotice'>Error: [alt] is not linked to [ckey]'s sticky ban!</span>", handle_whitespace = TRUE)
				return
			...
			if (!key)
				to_chat(usr, "<span class='adminnotice'>Error: [alt] link to [ckey]'s sticky ban disappeared.</span>", handle_whitespace = TRUE)
				return
		...
		if ("exempt")
			...
			if (!key)
				to_chat(usr, "<span class='adminnotice'>Error: [alt]'s link to [ckey]'s sticky ban disappeared.</span>", handle_whitespace = TRUE)
				return
			LAZYREMOVE(ban["keys"], alt)
			key["exempt"] = TRUE
		...
		if ("unexempt")
			...
			if (!key)
				to_chat(usr, "<span class='adminnotice'>Error: [alt] is not exempt from [ckey]'s sticky ban!</span>", handle_whitespace = TRUE)
				return
			...
			if (!key)
				to_chat(usr, "<span class='adminnotice'>Error: [alt]'s exemption from [ckey]'s sticky ban disappeared.</span>", handle_whitespace = TRUE)
				return
		...
		if ("timeout")
			if (!data["ckey"])
				return
			if (!SSdbcore.Connect())
				to_chat(usr, "<span class='adminnotice'>No database connection!</span>", handle_whitespace = TRUE)
				return
		...
		if ("revert")
			...
			if (!cached_ban)
				to_chat(usr, "<span class='adminnotice'>Error: No cached sticky ban for [ckey] found!</span>", handle_whitespace = TRUE)
			world.SetConfig("ban",ckey,null)
