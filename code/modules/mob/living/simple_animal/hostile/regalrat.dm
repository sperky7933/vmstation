...
/mob/living/simple_animal/hostile/regalrat/Initialize()
	...
	coffer.Grant(src)
	riot.Grant(src)
	var/list/mob/dead/observer/candidates = pollGhostCandidates("Do you want to play as the Royal Rat?", ROLE_SENTIENCE, null, FALSE, 100, POLL_IGNORE_SENTIENCE_POTION)
...
/datum/action/cooldown/coffer/Trigger()
	...
	switch(loot)
		...
		if(6 to 10)
			...
			to_chat(owner, "<span class='notice'>You find some leftover coins.</span>")
			for(var/i = 1 to rand(1,3))
				new pickedcoin(T)
		if(11)
			to_chat(owner, "<span class='notice'>You find a... Hmm... This coin doesn't look right.</span>")
